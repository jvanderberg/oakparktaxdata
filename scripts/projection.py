...
levyfit = pd.DataFrame(index=pd.Series(range(1999, 2039)),  data=np.exp(
    np.array(pd.Series(range(1999, 2039))) * loglevyfit[0] + loglevyfit[1]))

cpilevyfit = pd.DataFrame(index=pd.Series(range(1999, 2039)),  data=np.exp(
    np.array(pd.Series(range(1999, 2039))) * logcpifit[0] + loglevyfit[1]))

cpifit = pd.DataFrame(index=pd.Series(range(1999, 2039)),  data=np.exp(
    np.array(pd.Series(range(1999, 2039))) * logcpifit[0] + logcpifit[1]))
awifit = pd.DataFrame(index=pd.Series(range(1999, 2039)),  data=np.exp(
    np.array(pd.Series(range(1999, 2039))) * logawifit[0] + logawifit[1]))
...
rates = pd.DataFrame(index=pd.Series(("Levy","CPI (Prices)","AWI (Wages)"), name='Type'),data={"Rate": (round(100*levyrate,4),round(100*cpirate,4),round(100*awirate,4)), "Doubling Time": (round(levydouble), round(cpidouble), round(awidouble))})
rates.to_csv("bestfitrates.csv")

#create the detailed yearly projections table
final = pd.DataFrame(index=pd.Series(range(1999, 2039), name='Year'), data={
...
summary.to_csv('projectionssummary.csv')